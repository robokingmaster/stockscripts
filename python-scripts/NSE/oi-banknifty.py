...
def print_header(index="",currprice=0,nearest=0,bnf_open=0,bnf_high=0,bnf_low=0,expiry_date=None):
    if(currprice > bnf_open):
    ...
    header_string = strPurple( index + " => ") \
                    ...
                    + strLightPurple(" Today Open:") + strBold(str(bnf_open)) \
                    + strLightPurple(" Today Low:") + strBold(strRed(str(bnf_low))) \
                    + strLightPurple(" Today High:") + strBold(strGreen(str(bnf_high))) 
...
def print_oi_data(index_name, index_data, previous_data=None):    
    ...
    print('\033c')
    print_hr()
    print_header(index_name, bnf_ul, bnf_nearest, bnf_open, bnf_high, bnf_low, expiryDate)
    print_hr()
...
async def update_data():
    ...
    while True:
        ...
        print_oi_data("Bank Nifty", bank_nifty_oi_data, prev_bank_nifty_data)
